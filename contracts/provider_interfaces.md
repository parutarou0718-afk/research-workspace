...
## Gate 2 deterministic observation ports
...
Filesystem callbacks only emit immutable `RawFileEventDTO` values. They never
mutate `SourceObservation`, open a database Session, hash/copy a source, or
publish a formal DomainEvent. Reconciliation and candidate workers consume one
frozen plan and return immutable DTOs; Application validation and the unified
write coordinator own every persistent fact.
