...
def test_paper_page_empty_state_is_specific_and_actionable(qtbot):
    ...
    assert page.empty_title_label.text() == "还没有论文。"
    assert (
        page.empty_body_label.text()
        == "导入或新建第一篇论文，开始搭建你的研究工作台。"
    )
    assert page.empty_action_button.text() == "新建论文"
...
def test_paper_page_renders_cards_detail_and_search(qtbot):
    ...
    assert page.status_badge_label.text() == "进行中"
    assert page.research_analysis_title_label.text() == "研究分析"
    assert page.research_analysis_text_label.text() == "AI 尚未配置。"
    ...
    assert page.next_step_text_label.text() == "从这篇论文中记录一个想法。"
    assert page.create_idea_button.text() == "创建想法"
    ...
    page.list_view.setCurrentRow(0)
    page._update_actions()
    assert page.status_badge_label.text() == "已归档"
